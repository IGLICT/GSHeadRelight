...
def gen_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), gs_params=None, shs=None, image_names=None, albedo=False, **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    shs = torch.from_numpy(shs).to(device).float()
    ...
    image_modes = ['image_albedo'] if albedo else ['image']*len(shs)
    for sh, image_mode in zip(shs, image_modes):
        sh = sh.unsqueeze(0)
            ...
            for yi in range(grid_h):
                for xi in range(grid_w):
                    ...
                    if entangle == 'conditioning':
                        ...
                        ret_dict = G.synthesis_test(ws=w.unsqueeze(0).to(torch.float32), c=c[0:1], l=sh, noise_mode='const', gs_params=gs_params, image_mode=image_mode, dec_in=dec_out)
...
@click.option('--lighting_pattern', type=str, help='lighting pattern of video', default='')
...
@click.option('--sh_file_dir', type=str, help='dataset dir', default='')
@click.option('--envmap_dir', type=str, help='envmap dir', default='')
...
def generate_images(
    ...
    for seed in seeds:
        ...
        seeds_ = [seed]
        gen_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gs_params=gs_params, shs=shs, image_names=image_names, albedo=(lighting_pattern=='albedo'))
