...
class PointGenerator(nn.Module):
    def __init__(self,
        ...
        self.register_buffer("albedo_init", torch.tensor(torch.zeros([img_channels])))
        self.register_buffer("opacity_init", inverse_sigmoid(0.1 * torch.ones([1])))
        self.register_buffer("dc_rt_init", torch.zeros([l_dim]))
    ...
    def forward(self, x, ws, camera_cond=None, camera_swap_p=0.0):
        ...
        for i in self.block_res:
            ...
            if i == self.block_res[-1]:
                ...
                albedo = torch.cat([albedo, albedo_cur], dim=1)
                opacity = torch.cat([opacity,  opacity_cur], dim=1)
                dc_rt = torch.cat([dc_rt, dc_rt_cur], dim=1)
            ...
            # Intermediate layers (generate both anchor and output gaussians)
                ...
                albedo = torch.cat([albedo, albedo_cur_out], dim=1)
                opacity = torch.cat([opacity, opacity_cur_out], dim=1)
                dc_rt = torch.cat([dc_rt, dc_rt_cur_out], dim=1)
                ...
                albedo_anchor = torch.cat([albedo_anchor, albedo_cur], dim=1)
                opacity_anchor = torch.cat([opacity_anchor, opacity_cur], dim=1)
                dc_rt_anchor = torch.cat([dc_rt_anchor, dc_rt_cur], dim=1)
