...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        rgb_sh          =False, # Wether to use rgb sh labels 
        split_sh        =False, # Wether to use SPLiT sh labels (rgb -> mono)   
        with_bg         =True, # Wether to use rgb sh labels 
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        self._zipfile = None
        self.rgb_sh = rgb_sh
        self.split_sh = split_sh
        self.with_bg = with_bg
        assert not (split_sh and rgb_sh)
    ...
    def _load_raw_image(self, raw_idx):
        ...
        with self._open_file(fname) as f:
            ...
            # else:
                if self.with_bg:
                    image = np.array(PIL.Image.open(f).convert('RGB')) # .resize((self._resolution, self._resolution), PIL.Image.ANTIALIAS)
                else:
                    image_RGBA = np.array(PIL.Image.open(f))
                    alpha = image_RGBA[:,:,3]
                    image = image_RGBA[:,:,:3].astype(np.float32) * alpha[:,:,np.newaxis] / 255
    ...
    def _load_raw_labels(self):
        if self.rgb_sh:
            fname = 'dataset_rgb_sh_2.json'
        elif self.split_sh:
            fname = 'dataset_sh_2_ldr.json'
        else:
            fname = 'dataset.json'
        ...
        shs = dict(shs)
        if self.rgb_sh:
            shs = [np.array(shs[fname.replace('\\', '/')]).reshape((-1)) for fname in self._image_fnames]
        elif self.split_sh:
            shs = [np.array(shs[fname.replace('\\', '/')]).reshape((-1)) for fname in self._image_fnames]
        else:
            shs = [np.array(shs[fname.replace('\\', '/')]).reshape((-1)) for fname in self._image_fnames]
...
class ImageRGBAFolderDataset(Dataset):
    def __init__(self,
        path,                   # Path to directory or zip.
        rgb_sh          =False, # Wether to use rgb sh labels 
        split_sh        =False, # Wether to use SPLiT sh labels (rgb -> mono)   
