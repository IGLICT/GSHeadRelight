...
def init_dataset_kwargs(data):
    try:
        ...
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageRGBAFolderDataset', path=data, use_labels=True, max_size=None, xflip=False)
...
# Config options for GSHeadRelight
...
@click.option('--rgb_sh',    help='sh', metavar='BOOL', type=bool, required=False, default=False)
@click.option('--split_sh',    help='sh', metavar='BOOL', type=bool, required=False, default=False)
@click.option('--with_bg',    help='sh', metavar='BOOL', type=bool, required=False, default=True)
@click.option('--sh_dir',    help='sh', metavar='STR', type=str, required=True, default='')
...
def main(**kwargs):
    ...
    c.training_set_kwargs.rgb_sh = opts.rgb_sh
    c.training_set_kwargs.split_sh = opts.split_sh
    c.training_set_kwargs.with_bg = opts.with_bg
    ...
    c.G_kwargs.light_channels = 3 if opts.rgb_sh else 1
    ...
    c.loss_kwargs.update({
        'loss_custom_options':{
            ...
            'light_contrastive': 1.0,            # coeff. of pose contrastive loss
            'alpha': 1.0,            # coeff. of alpha loss
            ...
            'disable_background': not opts.with_bg,        # disable background generator
